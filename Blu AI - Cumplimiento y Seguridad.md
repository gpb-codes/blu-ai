...
- Gateway de IA multi-proveedor (Anthropic, Gemini, OpenAI, OpenRouter, fine-tunes propios sobre Qwen/Llama) + memoria compartida RAG con pgvector ([[Blu AI - Gateway y Modelos]], [[Blu AI - Memoria compartida]]).
- Este documento parte de la arquitectura descrita en el vault al 14-ago-2026, incluyendo el punto **en revisión** de migrar de PostgreSQL/Neon a Cloudflare D1 + Vectorize. Donde esa decisión no está cerrada, se documentan ambos escenarios y se marca como pendiente.
...
| Menores de edad | N/A — Blu no está dirigido a menores | Exclusión de servicio | Edad digital mínima 14-16 años según Estado miembro (LOPDGDD fija 14 en España) sin representante | 18 años sin representante legal | 18 años sin representante legal |
...
- **Base de datos:** hoy PostgreSQL gestionado en Neon (región no confirmada en el vault, típicamente AWS us-east-1 salvo configuración explícita) con pgvector para RAG. **En revisión** (bitácora 28-jul): migración a Cloudflare D1 (SQLite) + Vectorize.
...
|---|---|---|---|---|---|
| Neon (PostgreSQL + pgvector) | Encargado (infraestructura de datos) | Todos los datos de producto | AWS, región configurable (no confirmada) | DPA estándar disponible | Por firmar/confirmar |
| Cloudflare (D1 / Vectorize / Workers / Pages / FLUX) | Encargado (infraestructura, si se migra) | Datos de producto (si migra) + imágenes generadas | Red global tipo edge | Cloudflare Customer DPA | Por firmar/confirmar; aclarar implicancia de residencia por el modelo edge |
...
| Moonshot AI (Kimi) | Subencargado (IA) | Prompts/contenido | China (no confirmado públicamente) | No verificado | **Gap crítico** — no usar con datos personales hasta resolver |
| Together AI / Fireworks (fine-tunes Blu Light/Flash/Ultra) | Subencargado (hosting de modelos propios) | Prompts/contenido | No confirmado | No confirmado | Por firmar/confirmar |
...
- **Kimi (Moonshot AI):** con sede en China, sujeta a legislación china de seguridad nacional que puede habilitar acceso estatal a los datos; no publica ubicación de sus centros de datos ni ofrece, hasta donde se pudo verificar, DPA o cláusulas contractuales estándar equivalentes a las de Anthropic/OpenAI. Es el proveedor de mayor riesgo de cumplimiento del gateway. Tratarlo como **fuera del perímetro de datos personales** hasta que exista una evaluación legal específica y, en su caso, un DPA firmado.
- **Fine-tunes propios (Together AI / Fireworks, sobre Qwen/Llama):** aunque el modelo es "propio" de Blu, el hosting es de un tercero — se necesita el mismo tratamiento contractual que con cualquier otro subencargado de IA.
...
| Ambigüedad de residencia de datos por el modelo edge de Cloudflare D1 (si se migra) | Media | Medio | Definir explícitamente política de residencia por región de usuario antes de cerrar la migración |
...
- **Decisión final de base de datos:** cerrar si se migra a Cloudflare D1 + Vectorize o se mantiene Neon/PostgreSQL, y qué implica cada opción para la residencia de datos de usuarios europeos (el punto ya estaba marcado "en revisión" en [[Blu AI - Stack tecnologico]] y [[Blu AI - Memoria compartida]]).
...
- **Postura sobre Kimi/Moonshot AI y otros proveedores en China:** decidir si se excluyen completamente del tratamiento de datos personales, se limitan a tareas sin datos personales, o se investiga y firma un DPA formal antes de habilitarlos.
...
- **Verificación de edad mínima real** (hoy solo se documenta como política, sin mecanismo de verificación).
