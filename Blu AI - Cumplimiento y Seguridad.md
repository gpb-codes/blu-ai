...
- Gateway de IA multi-proveedor (Anthropic, Gemini, OpenAI, OpenRouter implementados; Kimi y el hosting de fine-tunes vía Together AI/Fireworks forman parte de la visión de producto pero no están confirmados como implementados — ver sección 5; **Ollama descartado del gateway, decisión 17-ago-2026**) + memoria compartida RAG con pgvector ([[Blu AI - Gateway y Modelos]], [[Blu AI - Memoria compartida]]).
...
- **Entidad legal (confirmado 17-ago-2026):** Blu operará como **una única entidad**, constituida en **México**, sirviendo los tres mercados (Chile, México, España) desde ahí — sin entidades separadas por país. Falta todavía la razón social exacta y la fecha de constitución (sección 9), pero la estructura ya está definida. Esto activa dos consecuencias directas: (1) al no tener establecimiento en la UE, Blu necesita representante legal en la UE (Art. 27 GDPR) para atender a usuarios de España; (2) para Chile, Blu opera como responsable extranjero bajo el ámbito extraterritorial de la Ley 21.719 — falta confirmar con abogado local si exige designar representante en Chile. Detalle en sección 8.
...
3. **Kimi / proveedores en China — postura confirmada (17-ago-2026):** se limita a tareas sin datos personales (ej. generación de código genérico), nunca prompts con datos identificables de usuarios. Sin residencia de datos pública, sujetos a legislación china de seguridad nacional (acceso potencial del Estado), sin DPA/SCC verificados — mientras no se firme un DPA y se complete una evaluación de transferencia, mantenerlo excluido del modo Auto por defecto y bloqueado a nivel de producto para cualquier flujo que pueda incluir datos personales, en las tres jurisdicciones cubiertas por este documento.
...
| Moonshot AI (Kimi) | Subencargado (IA) — mencionado en la visión de producto ([[Blu AI - Vision]], [[Blu AI - Memoria compartida]]), **no está entre los adapters implementados hoy** (`packages/ai-gateway` solo tiene Anthropic/Gemini/OpenAI/OpenRouter, verificado en el fork 17-ago-2026) | Solo tareas sin datos personales si se implementa (postura confirmada 17-ago-2026) — nunca prompts con datos identificables | China (no confirmado públicamente) | No verificado | Postura definida; **igual requiere DPA antes de implementar** cualquier uso, incluso sin datos personales |
...
| ~~Ollama~~ | **Descartado (decisión 17-ago-2026)** — no se implementará en el gateway; era solo parte de la visión inicial en [[Blu AI - Vision]] | N/A | N/A | N/A | Cerrado — no requiere DPA. Pendiente actualizar el diagrama en Vision.md para quitarlo |
...
- **Entidad legal confirmada (17-ago-2026): Blu no constituirá entidad en Chile** — operará como una única sociedad mexicana (sección 0). No aplican, entonces, las obligaciones de facturación electrónica ante el SII que sí aplicarían con una entidad chilena. En cambio, Blu opera como responsable de datos **extranjero** bajo el ámbito de aplicación extraterritorial de la Ley 21.719 (aplica a quien procese datos de personas en Chile aunque no esté domiciliado ahí, de forma similar al GDPR). **Gap:** no se pudo confirmar con las fuentes disponibles si la ley (o su reglamento, aún pendiente de publicarse antes de dic-2026) exige designar un representante local en Chile para responsables extranjeros — verificar con abogado local (sección 9).

### México

- **Jurisdicción de origen de Blu (confirmado 17-ago-2026):** la entidad matriz —y única— de Blu se constituye aquí (sección 0). Esto simplifica el cumplimiento en este mercado (Blu "es de casa"), pero significa que Chile y España se tratan como mercados donde opera sin presencia local — ver representante en la UE y gap de representante en Chile.
...
- **Hosting de Ollama — cerrado (17-ago-2026):** decidido no usarlo; se descarta del gateway. Ya no es un gap, solo queda actualizar el diagrama de [[Blu AI - Vision]] para reflejarlo (sección 5).
