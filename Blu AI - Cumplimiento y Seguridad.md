...
> Arquitectura de cumplimiento legal y seguridad de Blu AI para **Chile, México y España**. Cierra la tarea de [[Blu AI - Kanban|Kanban]] *"Documentar cumplimiento legal de la arquitectura"*. Es un documento de arquitectura y política — no incluye implementación de código. Contiene una sección final de huecos y decisiones pendientes que el equipo debe resolver antes de tratarlo como aplicado en producción.
>
...
- Plataforma SaaS multi-dispositivo (Web, Windows, macOS, Android, iOS, extensión Chrome) — ver [[Blu AI - Vision]] y [[Blu AI - Stack tecnologico]].
- Mercados objetivo: **Chile, México y España**, con perfiles de cliente mayoritariamente hispanohablantes ([[Blu AI - Clientes ideales]]).
- Modelo de negocio freemium + BYOK ($10) + Créditos ($30), con Stripe como procesador de pagos ([[Blu AI - Planes y monetizacion]]).
- Gateway de IA multi-proveedor (Anthropic, Gemini, OpenAI, OpenRouter implementados; Kimi, Ollama y el hosting de fine-tunes vía Together AI/Fireworks forman parte de la visión de producto pero no están confirmados como implementados — ver sección 5) + memoria compartida RAG con pgvector ([[Blu AI - Gateway y Modelos]], [[Blu AI - Memoria compartida]]).
...
- No hay en el vault información sobre la(s) entidad(es) legal(es) bajo la(s) que opera Blu (razón social, país de constitución, si hay una entidad por país o una matriz). Es una entrada crítica pendiente (sección 9) porque de eso depende, por ejemplo, si Blu necesita representante legal en la UE (Art. 27 GDPR) o inscripción ante autoridades locales.
...
- **Storage de archivos:** S3-compatible, proveedor y región no confirmados en el vault (gap, ver sección 9).
...
3. **Kimi / proveedores en China:** sin residencia de datos pública, sujetos a legislación china de seguridad nacional (acceso potencial del Estado), sin DPA/SCC verificados. Recomendación: **no enrutar prompts con datos personales identificables de usuarios hacia Kimi u otros proveedores chinos** hasta tener un DPA firmado y una evaluación de transferencia completa; si se usa, limitarlo a tareas sin datos personales (por ejemplo, generación de código genérico) y excluirlo del modo Auto por defecto para las tres jurisdicciones cubiertas por este documento.
...
| Datos de pago | Blu no almacena PAN; retención a cargo de Stripe según sus propias políticas (procesador con certificación PCI-DSS) | Gestionado por Stripe |
...
| Logs retenidos por proveedores de IA (Anthropic, OpenAI, etc.) | Según política propia de cada proveedor (p. ej. Anthropic retiene ~30 días para confianza y seguridad salvo que se contrate *Zero Data Retention*) | Fuera del control directo de Blu — documentar en el registro de subencargados (sección 4) |
| Datos heredados del bot de WhatsApp (descontinuado) | Por definir | **Gap** — falta decidir si se conservan, migran o eliminan (ver sección 9) |
...
| Storage S3-compatible | Encargado (archivos, mini-apps generadas) | Archivos subidos por usuarios | No confirmado | No confirmado | **Gap** — falta decidir proveedor |
...
| Moonshot AI (Kimi) | Subencargado (IA) — mencionado en la visión de producto ([[Blu AI - Vision]], [[Blu AI - Memoria compartida]]), **no está entre los adapters implementados hoy** (`packages/ai-gateway` solo tiene Anthropic/Gemini/OpenAI/OpenRouter, verificado en el fork 17-ago-2026) | Prompts/contenido (si se implementa) | China (no confirmado públicamente) | No verificado | **Gap crítico** — no implementar ni enrutar datos personales hasta resolver |
...
| Ollama | Sin confirmar — mencionado solo en el diagrama de [[Blu AI - Vision]], ausente de todo lo demás en el vault y del código auditado | Prompts/contenido, si corre en infraestructura propia de Blu no saldría de su perímetro | N/A si es autohospedado (no confirmado) | N/A si es autohospedado (no confirmado) | **Gap** — confirmar con el equipo si es self-hosted (sin relación de subencargado) o gestionado por un tercero (requeriría DPA) |
...
| Proveedor de email transaccional | Encargado | Direcciones de correo, contenido de notificaciones | No identificado en el vault (¿SendGrid/Resend/Postmark?) | No confirmado | **Gap** — falta identificar proveedor |
...
- **Kimi (Moonshot AI):** aparece en la visión de producto (Vision.md, Memoria compartida.md) como parte del pitch de "cambiar de modelo sin perder contexto", pero **no está entre los adapters implementados hoy** — `Gateway y Modelos.md` documenta como implementado solo Anthropic/Gemini/OpenAI/OpenRouter (verificado en el fork, 17-ago-2026). Se documenta igual aquí porque forma parte de la visión de producto y podría implementarse más adelante. Con sede en China, sujeta a legislación china de seguridad nacional que puede habilitar acceso estatal a los datos; no publica ubicación de sus centros de datos ni ofrece, hasta donde se pudo verificar, DPA o cláusulas contractuales estándar equivalentes a las de Anthropic/OpenAI. Sería el proveedor de mayor riesgo de cumplimiento del gateway si se implementa. Tratarlo como **fuera del perímetro de datos personales** hasta que exista una evaluación legal específica y, en su caso, un DPA firmado — y volver a pasar por esta sección antes de habilitarlo en producción.
- **Fine-tunes propios (Together AI / Fireworks, sobre Qwen/Llama):** aunque el modelo es "propio" de Blu, el hosting es de un tercero — se necesita el mismo tratamiento contractual que con cualquier otro subencargado de IA. Igual que Kimi, **no está confirmado como implementado** en el fork auditado el 17-ago-2026 (el propio `Gateway y Modelos.md` lista los adapters reales sin incluirlo) — tratar como pendiente de contrato antes de activarlo, no como algo ya en producción.
- **Ollama:** aparece en el diagrama de arquitectura de [[Blu AI - Vision]] como parte del ecosistema de modelos, pero no está documentado en ningún otro lugar del vault ni confirmado en el código auditado. Ollama normalmente corre **autohospedado** (en la propia infraestructura de Blu, sin enviar datos a un tercero) — si es así, no generaría una relación de subencargado y no necesitaría DPA. Pero esto **no está confirmado**: si en cambio se usa vía un proveedor de hosting gestionado, aplicaría el mismo tratamiento contractual que a los demás proveedores de IA. Aclarar con el equipo antes de asumir cualquiera de los dos escenarios (gap, sección 9).
...
- **En reposo:** cifrado nativo de la base de datos (Neon/Cloudflare D1), cifrado de storage de archivos, y cifrado de API keys BYOK con **AES-256-GCM** (ya documentado en [[Blu AI - Stack tecnologico]]). Recomendación adicional: mover la clave maestra de un simple valor de entorno a un gestor de secretos/KMS dedicado (AWS KMS, Cloudflare Secrets Store o equivalente) con rotación periódica, en vez de una variable de entorno estática.

### Gestión de acceso

- Extender el modelo de permisos por proyecto (owner/admin/editor/viewer, ya definido en [[Blu AI - Planes y monetizacion]]) a un modelo de **acceso interno del equipo**: mínimo privilegio para quien accede a producción, roles diferenciados para soporte vs. ingeniería, y **MFA obligatorio** para cualquier cuenta con acceso administrativo o a la base de datos de producción.
...
| Ausencia de responsable de privacidad/DPO designado | Alta hoy | Medio (bloquea respuesta a autoridades y a titulares) | Designar responsable (sección 9) |
...
| Datos heredados del bot de WhatsApp descontinuado sin política definida | Media | Medio | Decidir migración/eliminación explícita (sección 9) |
...
- Blu **no debe almacenar ni procesar directamente** números de tarjeta (PAN), CVV ni fecha de expiración: usar Stripe Checkout/Elements para que la captura ocurra en el frontend de Stripe o en campos tokenizados, de forma que Blu nunca "toque" el dato de tarjeta en su propio servidor.
- Con ese diseño, el alcance de cumplimiento **PCI-DSS** de Blu se reduce al nivel más bajo (equivalente a SAQ A), porque el manejo de datos de tarjeta queda tercerizado en Stripe, que mantiene su propia certificación PCI-DSS (versión actual 4.0.1).
...
- Documentar esto explícitamente en política interna: "ningún servicio de Blu debe recibir, almacenar o loguear el número de tarjeta completo".
...
- Protección al consumidor (Ley del Consumidor / SERNAC) aplica a los términos de suscripción: derecho a retracto en compras a distancia, claridad en el cobro recurrente y en la cancelación.
- Si Blu constituye entidad en Chile, considerar obligaciones de facturación electrónica ante el SII.
...
- Requisitos clave: base legal explícita por tratamiento (Art. 6), Registro de Actividades de Tratamiento (Art. 30), evaluación de impacto (DPIA) recomendable dado que Blu combina IA a gran escala con memoria compartida entre usuarios — perfil de riesgo que suele justificar una DPIA aunque no sea automáticamente obligatoria; evaluar la designación de un Delegado de Protección de Datos (DPO) — no siempre obligatorio por umbral pero recomendable dado el tratamiento a gran escala de datos mediante IA; mecanismo de transferencia válido para todo proveedor fuera del EEE (sección 2); notificación de brechas en 72 horas a la AEPD; derechos de acceso, rectificación, supresión, limitación, portabilidad y oposición.
- **EU AI Act — Artículo 50 (obligaciones de transparencia):** entró en aplicación el **2 de agosto de 2026** (hace apenas unos días respecto a la fecha de este documento). Exige que los sistemas de IA que interactúan directamente con personas —como el chat de Blu— estén diseñados para que el usuario sepa que está interactuando con una máquina, salvo que sea evidente por el contexto. Como proveedor del sistema, Blu debe incorporar ese aviso de forma visible (por ejemplo, en el propio producto, no solo en términos legales) y no puede diluirlo con patrones de diseño engañosos.
- Si Blu no tiene establecimiento en la UE pero ofrece el servicio a residentes de España, corresponde evaluar la designación de un representante en la UE (Art. 27 GDPR) — depende de la entidad legal bajo la que opere Blu (gap, sección 9).

## 9. Huecos y decisiones pendientes del equipo

- **Entidad legal:** no está definido en el vault bajo qué razón social y país de constitución opera Blu, ni si habrá una entidad separada por mercado (Chile/México/España) o una matriz única. De esto depende, por ejemplo, si se necesita representante en la UE (Art. 27 GDPR) o inscripción local.
- **Política de residencia de datos para la migración a Cloudflare D1** (ya no es "si se migra" — la decisión está confirmada desde el 17-ago-2026, ver [[Blu AI - Stack tecnologico]] y [[Blu AI - Bitacora]]): falta definir qué implica el modelo edge de D1 para la residencia de datos de usuarios europeos, y completar la implementación (el fork auditado el 17-ago-2026 seguía en Prisma/PostgreSQL) antes de la fecha límite de la Ley 21.719 de Chile (1-dic-2026).
- **Confirmar el modelo de hosting de Ollama:** aclarar si corre autohospedado en infraestructura de Blu (sin relación de subencargado) o vía un proveedor externo gestionado (que sí requeriría DPA) — mencionado en [[Blu AI - Vision]] pero no resuelto en ningún otro lugar del vault.
- **Proveedor de storage de archivos** (S3-compatible): no identificado en el vault — falta elegir proveedor y región.
- **Proveedor de email transaccional:** no identificado en el vault — falta elegir y firmar DPA.
- **Postura sobre Kimi/Moonshot AI y otros proveedores en China:** decidir si se excluyen completamente del tratamiento de datos personales, se limitan a tareas sin datos personales, o se investiga y firma un DPA formal antes de habilitarlos. Nota (verificado 17-ago-2026): ni Kimi ni el hosting de fine-tunes vía Together AI/Fireworks están implementados todavía en el código — esta decisión debe cerrarse antes de implementarlos, no después.
- **Allowlist de modelos permitidos detrás de OpenRouter**, para no exponer el catálogo completo sin control de subencargados.
- **Designar responsable/DPO de privacidad** y punto de contacto ante AEPD (España), la futura Agencia de Protección de Datos (Chile, desde dic-2026) y la Secretaría Anticorrupción y Buen Gobierno (México).
- **Firma efectiva de los DPAs** listados en la sección 4 con cada proveedor — hoy son "por confirmar", no firmados.
- **Automatizar el flujo de eliminación de datos** (sección 3) — hoy no hay evidencia de que exista.
- **Definir qué pasa con los datos del bot de WhatsApp descontinuado** (migrar, anonimizar o eliminar, y en qué plazo).
...
Relacionado: [[Blu AI - Vision]] · [[Blu AI - Stack tecnologico]] · [[Blu AI - Gateway y Modelos]] · [[Blu AI - Memoria compartida]] · [[Blu AI - Roadmap y estado]] · [[Blu AI - Kanban]]
