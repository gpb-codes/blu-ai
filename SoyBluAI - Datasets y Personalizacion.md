...
# SoyBluAI — Datasets y Personalización

> Cómo SoyBluAI junta señales de uso para personalizar respuestas por usuario y para entrenar/afinar los tiers propios (SoyBluAI Light / Flash / Ultra). Formato de almacenamiento: **JSONL, no JSON.**

## Qué es

Todo lo que SoyBluAI aprende de un usuario o de una conversación (preferencias, feedback, memoria persistente, ejemplos para fine-tuning) se registra como una serie de **eventos independientes**, uno por línea, en archivos `.jsonl`. No es un único documento JSON gigante que se reescribe — es un log append-only de registros pequeños.

Alimenta directamente:
- [[SoyBluAI - Memoria compartida]] (preferencias y contexto persistente por usuario/proyecto)
- [[SoyBluAI - Gateway y Modelos]] (datos de entrenamiento para el fine-tuning de SoyBluAI Light/Flash/Ultra, y señal para el clasificador del modo Auto)
...
| `finetune_example` | Ejemplo conversacional completo, curado para entrenar un tier propio | Fine-tuning SoyBluAI Light/Flash/Ultra |
...
{"event": "finetune_example", "messages": [{"role": "system", "content": "Eres SoyBluAI, un asistente directo y sin rodeos."}, {"role": "user", "content": "Resume este PR en 3 líneas."}, {"role": "assistant", "content": "1) Agrega auth JWT+refresh. 2) Cierra el gap de BYOK a medias. 3) No toca el frontend."}], "metadata": {"tier_objetivo": "SoyBluAI Flash", "fuente": "conversacion_real_anonimizada", "fecha": "2026-08-17"}}
```
...
Estos archivos van a contener datos de comportamiento de usuarios reales — hay que tratarlos con las mismas reglas que el resto de datos personales en [[SoyBluAI - Cumplimiento y Seguridad]]:
...
- Si el volumen crece mucho, evaluar mover `events/` de archivos planos a un bucket (Cloudflare R2, ya elegido para storage — ver [[SoyBluAI - Cumplimiento y Seguridad]]) particionado por fecha, en vez de archivos locales.

Ver también: [[SoyBluAI - Memoria compartida]], [[SoyBluAI - Gateway y Modelos]], [[SoyBluAI - Cumplimiento y Seguridad]].
